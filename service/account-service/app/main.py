...
async def signup(request_data: SignUpRequest, http_request: Request):
    ...
    try:
        ...
        logger.info(f"📋 회원가입 입력값 검증: user_id={request_data.user_id}, password_length={len(password) if password else 0}, company_id={request_data.company_id}")
